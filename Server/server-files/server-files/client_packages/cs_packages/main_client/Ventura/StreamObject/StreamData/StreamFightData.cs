...
{
    class StreamFightData : Events.Script
    {
        private string event_name = main_client.Ventura.Constant.Constant.table_event_name[((int)main_client.Ventura.Constant.Constant.UpdateFlagsClient.WorldFightData >> 2) - 1];
        private string data_name = main_client.Ventura.Constant.Constant.table_data_name[((int)main_client.Ventura.Constant.Constant.UpdateFlagsClient.WorldFightData >> 2) - 1];
        ...
        private void ProcessTick()
        {
            ...
            foreach (var client in A.Entities.Players.All)
            {
                ...
                {
                    var data = JsonConvert.DeserializeObject<StreamObject.WorldData.WorldFightData>((string)client.GetSharedData(data_name), Constant.Constant.setting);
                    if (data != null)
                        data.StreamIn();
                    tick = 0;
                }
                ...
                {
                    JsonConvert.DeserializeObject<StreamObject.WorldData.WorldFightData>(client.GetData<string>(data_name)).StreamIn();
                    tick = 0;
                }
        ...
        private void UpdateSharedData(object[] args)
        {
            ...
            //Chat.Output("Initialize Body "+args[1].ToString()+ " " + event_name + " "+ data_name);
            /*A.Player target_player = A.Entities.Players.All.Find(x => x.RemoteId == Convert.ToUInt16(args[0]));
            A.Vehicle target_vehicule = A.Entities.Vehicles.All.Find(x => x.RemoteId == Convert.ToUInt16(args[0]));
        ...
        private void PlayerStreamIn(A.Player player, string json)
        {
            ...
            JsonConvert.DeserializeObject<StreamObject.WorldData.WorldFightData>(json).StreamIn();
            active_tick = true;
        }
