...
{
    class StreamClothData : Events.Script
    {
        private string event_name = main_client.Ventura.Constant.Constant.table_event_name[((int)main_client.Ventura.Constant.Constant.UpdateFlagsClient.WorldClothData >> 2) - 1];
        private string data_name = main_client.Ventura.Constant.Constant.table_data_name[((int)main_client.Ventura.Constant.Constant.UpdateFlagsClient.WorldClothData >> 2) - 1];
        ...
        private void ProcessTick()
        {
            ...
            foreach (var client in A.Entities.Players.All)
            {
                ...
                {
                    WorldData.WorldClothData data = JsonConvert.DeserializeObject<StreamObject.WorldData.WorldClothData>(client.GetSharedData(data_name).ToString(), Constant.Constant.setting);
                    if (data != null)
                        data.StreamIn();
                    tick = 0;
                }
                ...
                {
                    JsonConvert.DeserializeObject<StreamObject.WorldData.WorldClothData>(client.GetData<string>(event_name)).StreamIn();
                    tick = 0;
                }
            }
        }
        ...
        private void PlayerStreamIn(A.Player player, string json)
        {
            ...
            JsonConvert.DeserializeObject<StreamObject.WorldData.WorldClothData>(json).StreamIn();
            active_tick = true;
        }
